"""Installed CLI for watching an agent play one Wordle episode."""
...
from .backends import GenConfig, TransformersBackend
from .env.wordle import FEEDBACK_WIN, ILLEGAL, WordleEnv
from .knowledge import Knowledge
from .protocol import build_messages, render_constraint_summary
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--answer", default=None, help="5 字母答案；省略則從 eval_200 抽")
    parser.add_argument("--model", default="Qwen/Qwen2.5-1.5B-Instruct")
    parser.add_argument("--adapter", default=None)
    parser.add_argument("--device", default=None, help="auto/cpu/cuda（預設自動）")
...
def main() -> int:
    ...
    adapter_text = f" + adapter {args.adapter}" if args.adapter else ""
    print(f"載入模型 {args.model}{adapter_text} …")
    ...
    while not state.done:
        ...
        violation_text = f"  ⚠ {'、'.join(record.violations)}" if record.violations else ""
        repeat_text = "  ⚠ 重複猜測" if record.is_repeat else ""
        print(f"解析：{record.guess!r}（{record.parse_outcome}）{violation_text}{repeat_text}")
